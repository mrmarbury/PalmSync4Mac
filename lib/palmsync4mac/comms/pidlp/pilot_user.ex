defmodule PalmSync4Mac.Comms.Pidlp.PilotUser do
  ...
  typedstruct do
    ...
    field(:password_length, non_neg_integer(),
      doc: "Length of the password used to secure the Palm. This must match the actual length of the :password field"
    )

    field(:username, String.t(),
      doc: "Human readable name of the device"
    )

    field(:password, String.t(), doc: "The password used to secure the Palm device")
    field(:user_id, non_neg_integer(), doc: "Id of the Palm user/device. This is unique for each palm since it will be set on first hot sync. If this is changed, then the Palm is seen as a new device")
    ...
    field(:last_sync_pc, non_neg_integer(),
      doc: "Id or name of the PC used in the last sync. Should be set to the current hostname when writing. Will contain the last used hostname when reading.")
